...
using the *@Spring BootTest* annotation and invoke the APIs using *RestAssured*.
...
* We have annotated the test class with the *@Spring BootTest* annotation together with the webEnvironment config, so that the test will run by starting the entire application on a random available port.
